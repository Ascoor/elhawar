...
class EmployeesDataTable extends BaseDataTable
{
    ...
    {
        $roles = Role::where('name', '<>', 'client')->where('name', '<>', 'member')->get();
        ...
        return datatables()
            ...
            ->editColumn('name', function ($row) use ($roles) {
                ...
                return  '<div class="row truncate"><div class="col-sm-1 col-xs-1"></div><div class="col-sm-7 col-xs-6"><a href="' . route('admin.employees.show', $row->id) . '">' . ucwords($row->name) . '</a><br><span class="text-muted font-12">' . $designation . '</span></div></div>';
