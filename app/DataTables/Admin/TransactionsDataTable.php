...
class TransactionsDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) use ($user) {
                ...
                if($user->cans('edit_inventories'))
                  $action .= '<li><a href="' . route("admin.stocks.edit", $row->id) . '"><i class="fa fa-pencil-square-o"></i> ' .  __('app.show')  . '</a></li>';
                $action .= '</ul> </div>';
    ...
    public function query(StockTransaction $model)
    {
        $request = $this->request();
         ...
         ->select('stock_transactions.*','products.name','product_inventories.inventory','inventories.name as inventory_name')
        ...
        if($request->inventory !== null && $request->inventory !== 'all'){
            $model = $model->where('product_inventories.inventory','=',$request->inventory);
        }
        if($request->startDate !== null ){
            $model = $model->where('stock_transactions.date','>=',$request->startDate);
        }
        if($request->endDate !== null ){
            $model = $model->where('stock_transactions.date','<=',$request->endDate);
        }
    ...
    protected function getColumns()
    {
        ...
        $dsData =  [
            ...
            __('modules.stocks.inventory')  => ['data' => 'inventory_name', 'name' => 'inventory_name'],
