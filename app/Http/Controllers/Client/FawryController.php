...
class FawryController extends Controller
{
    ...
    public function paymentWithFawry(Request $request, $invoiceId): JsonResponse
    {
        ...
        $response['chargeRequest']['returnUrl'] = route('client.fawryPaymentReturn');
        ...
        $return_url = route('client.fawryPaymentReturn');
        $signature_string =
              $merchantCode
            . $merchantRefNum
            . ($profile_id  ?? "")
    ...
    public function fawryPaymentReturn(Request $request) : RedirectResponse
    {
        if($request->has('merchantRefNumber'))
        {
            ...
            $redirectRoute = 'client.invoices.index';
        ...
        if($request->has('statusCode') && $request['statusCode'] ==200 && $validate_signature == 1)
        {
            ...
            Session::put('error','Payment was not successful - Reason:' . $request['statusDescription'] ?? $request['description']);
