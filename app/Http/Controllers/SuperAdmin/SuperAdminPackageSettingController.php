    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->has('module_in_package') && !is_null($package)){
            ...
            foreach($companies as $company){
                ...
                $memberModules = ['projects', 'tickets', 'invoices', 'estimates', 'events', 'tasks', 'messages', 'payments', 'contracts', 'notices'];
                foreach ($moduleInPackage as $module) {
                    ...
                    if(in_array($module, $memberModules)){
                        $moduleSetting = new ModuleSetting();
                        $moduleSetting->company_id = $company->id;
                        $moduleSetting->module_name = $module;
                        $moduleSetting->status = 'active';
                        $moduleSetting->type = 'member';
                        $moduleSetting->save();
                    }
        ...
        return Reply::success(__('$memberModules'));
