...
class SuperAdminStripeSettingsController extends SuperAdminBaseController
{
    ...
    public function update(UpdateRequest $request) {
        ...
        //Fawry Settings
        $stripe->sandbox_merchant_code = $request->sandbox_merchant_code ?? $stripe->sandbox_merchant_code;
        $stripe->sandbox_merchant_hash = $request->sandbox_merchant_hash ?? $stripe->sandbox_merchant_hash;
        $stripe->sandbox_plugin_url = $request->sandbox_plugin_url ?? $stripe->sandbox_plugin_url;

        $stripe->live_merchant_code = $request->live_merchant_code ?? $stripe->live_merchant_code;
        $stripe->live_merchant_hash = $request->live_merchant_hash ?? $stripe->live_merchant_hash;
        $stripe->live_plugin_url = $request->live_plugin_url ?? $stripe->live_plugin_url;

        $stripe->fawry_mode = $request->fawry_mode ?? $stripe->fawry_mode;
        ($request->fawry_status) ? $stripe->fawry_status = 'active' : $stripe->fawry_status = 'deactive';
