...
class ModuleSettingsController extends AdminBaseController
{
    ...
    public function index(Request $request) {
        ...
        if($request->has('type')){
            ...
            elseif($request->get('type') == 'member'){
                $this->modulesData = ModuleSetting::where('type', 'member')->whereIn('module_name', $moduleInPackage)->get();
                $this->type = 'member';
            }
        }
        else{
    ...
    public function update(Request $request){
        ...
        switch ($setting->type) {
            ...
            case "client":
                ...
                break;
            case "member":
                $memberModules = ModuleSetting::where('type', 'member')->where('status', 'active')->pluck('module_name')->toArray();

                if($setting->module_name == 'timelogs' && $request->status == 'active') {
                    if(in_array('tasks', $memberModules) == false) {
                        return Reply::error(__('messages.enableTimeLogModuleMessage'), 'module_dependent');
                    }
                }

                if($setting->module_name == 'tasks' && $request->status == 'deactive') {
                    if(in_array('timelogs', $memberModules)) {
                        return Reply::error(__('messages.disableTasksModuleMessage'), 'module_dependent');
                    }
                }
                break;
