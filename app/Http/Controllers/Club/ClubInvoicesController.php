...
class ClubInvoicesController extends ClubBaseController
{
    ...
    {
        return view('club.invoices.index', $this->data);
    }

    public function create()
    {
        $member=memberDetails::where('user_id',$this->user->id)->first();
        ...
        $invoices = Invoice::leftJoin('projects', 'projects.id', '=', 'invoices.project_id')
            ...
            ->select('invoices.id', 'projects.project_name', 'invoices.invoice_number', 'currencies.currency_symbol', 'currencies.currency_code', 'invoices.total', 'invoices.issue_date', 'invoices.status','users.name')
//            ->where(function ($query) {
//
//                $query->where('projects.client_id', $this->user->id)
//                    ->orWhereIn('invoices.client_id', $familyMembersIds);
//            })
                ->whereIn('invoices.client_id' , $familyMembersIds)
        ...
        return DataTables::of($invoices)
            ->addColumn('action', function ($row) {
                return '<a href="' . route('club.invoices.download', $row->id) . '" data-toggle="tooltip" data-original-title="Download" class="btn  btn-sm btn-outline btn-info"><i class="fa fa-download"></i> '.__('app.download').'</a>';
            })
            ->editColumn('name', function ($row) {
                return $row->name != '' ? $row->name : '--';
            })
            ->editColumn('invoice_number', function ($row) {
                return '<a style="text-decoration: underline" href="' . route('club.invoices.show', $row->id) . '">' . $row->invoice_number . '</a>';
            })
    ...
    public function show($id)
    {
        ...
        return view('club.invoices.show', $this->data);
    }
...
//            'credit_note' => 0
    ...
    public function offlinePayment(Request $request)
    {
        ...
        return \view('club.invoices.offline-payment', $this->data);
    }
    ...
    public function offlinePaymentSubmit(OfflinePaymentRequest $request)
    {
        ...
        return Reply::redirect(route('club.invoices.show', $checkAlreadyRequest->id));
