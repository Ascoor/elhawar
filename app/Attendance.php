...
class Attendance extends BaseModel
{
    ...
    public static function countDaysPermissionsByUser($startDate, $endDate, $userId){
        $totalPermissions = DB::select('SELECT count(DISTINCT DATE(attendances.clock_in_time) ) as PermissionCount from attendances where DATE(attendances.clock_in_time) >= "' . $startDate . '" and DATE(attendances.clock_in_time) <= "' . $endDate . '" and user_id="' . $userId . '" and permission = "yes" ');
        return $totalPermissions = $totalPermissions[0]->PermissionCount;
    }
