    ...
    public function run()
    {
        ...
        try {
            ...
            factory(\App\Project::class, 10)->create()->each(function ($project) use ($company, $employee, $client, $currency, $taskboard) {
                ...
                $timeLog->start_time = Carbon::createFromFormat('Y-m-d H:i:s', $timeLog->start_time, 'Africa/Cairo')->setTimezone('UTC');
                $timeLog->end_time = \Carbon\Carbon::now()->subDays($attDay)->format('Y-m-d') . ' ' . Carbon::parse('04:15 PM')->format('H:i:s');
                $timeLog->end_time = Carbon::createFromFormat('Y-m-d H:i:s', $timeLog->end_time, 'Africa/Cairo')->setTimezone('UTC');
