                                ...
                                <div class="table-responsive">
                                    <table class="table table-borderless" id="earning-table">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-right text-uppercase">{{ $global->currency->currency_symbol.$salaryBasic }}</td>
                                            <input type="hidden" class="form-control" id="basic-salary" name="basic_salary" value="{{ $salarySlip->basic_salary }}">
                                        </tr>
                                        <tr>
                                            <td>@lang('app.sal.allowances')</td>
                                            <td class="text-right text-uppercase">{{ $global->currency->currency_symbol.$salaryAllowances }}</td>
                                        </tr>
                                        <tr>
                                            <td>@lang('app.sal.increment')</td>
                                            <td class="text-right text-uppercase">{{ $global->currency->currency_symbol.$salaryIncremenetes }}</td>
                                        </tr>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-borderless" id="deduction-table">
                                        ...
                                        @foreach ($deductions as $key=>$item)
                                            <tr>
                                                <td>
                                                {{-- rola  added translation to TDS --}}
                                                @if($key=='TDS')
                                                @lang('app.tds')

                                                {{-- 'rola' --}}
                                                @elseif($key == 'Total Penalty Duration')
                                                @lang('app.TotalPenaltyDuration')
                                                @else
                                                {{ $key }}
                                                @endif
                                                ...
                                                <td class="text-right">
                                                    ...
                                                    <input type="text" class="form-control" name="deductions[]" value="{{ $item }}" readonly>
                                                    @else
                                                    <input type="hidden" class="form-control" name="deductions_name[]" value="{{ $key }}">
                                    ...
                                    {{-- @foreach ($penality_durations as $penality_duration)
                                        <tr>
                                            <td class="">
                                                ...
                                                <label for="">{{ $penality_duration->penalty_name}} @lang('modules.employees.duration')</label>
                                             <select name="duration" id="duration" data-placeholder="@lang('modules.employees.duration')" class="form-control " >
                                                <option value="{{ $penality_duration->amount }}">{{ ucwords($penality_duration->amount) }}</option>
                                                @foreach($p_durations_select as $duration)
                                           ...
                                           <div class="col-md-6 d-flex">
                                            <label for="">@lang('modules.employees.MonthsDays')
                                                ...
                                                <select name="MonthsDays" id="MonthsDays" class="form-control">
                                                   
                                                    <option value="{{ $penality_duration->currency=='days' ? 'Days' : 'Months' }}">{{  $penality_duration->currency=='days' ? 'Days' : 'Months' }}</option>
                                                    <option value="{{ $penality_duration->currency!='days' ? 'Days' : 'Months' }}">{{   $penality_duration->currency!='days' ? 'Days' : 'Months' }}</option>
